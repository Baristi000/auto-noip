...
def renew(driver, host:str, ip: str):
    ...
    sleep(1)
    # conts job
    driver.find_element_by_xpath('//input[@id="name"][@placeholder="myhost"]').send_keys(host.split(".")[0])
    driver.find_element_by_xpath('//input[@data-cy="ipv4-address"]').send_keys(ip)
...
def renewAll(driver, days:int, ip:str):
    hosts = getAllHostName(driver)
    for host in hosts:
        print("check host:\t", host)
        if int(getExpireDateFromHost(driver, host)) <= days:
            print("EXPIRED!")
